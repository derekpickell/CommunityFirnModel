...
# total_compactions = np.nansum(compaction_values, axis=1)
# total_compactions = total_compactions*-1
total_compactions = [-np.sum(x) for x in compaction_values]
ax3.plot(decimal_year_values[1:], total_compactions[1:])
ax3.set_ylabel("m")
ax3.set_title("Designated column compaction since last timestep")
total_compaction_cumsum = np.cumsum(total_compactions[1:])
...
# x =15
# fiz, az = plt.subplots()
# az.plot(single_compaction_date, depths)
# ay = az.twiny()
